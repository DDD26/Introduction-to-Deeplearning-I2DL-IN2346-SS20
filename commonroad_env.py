...
from commonroad_rl.gym_commonroad.constants import PATH_PARAMS, VEHICLE_PARAMS
...
class CommonroadEnv(gym.Env):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not test_env and not play:
            ...
            for fn in fns:
                with open(fn, "rb") as f:
                    self.all_problem_dict[
                        os.path.basename(fn).split(".")[0]
                    ] = pickle.load(f)
            self.is_test_env = False
            LOGGER.info(
                f"Training on {train_reset_config_path} "
            ...
            LOGGER.info(
                f"Testing on {test_reset_config_path} "
                f"with {len(self.all_problem_dict.keys())} scenarios"
            )
        ...
        self.action_space = gym.spaces.Box(
            low=-action_high, high=action_high, dtype="float32"
        )
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, float, bool, dict]:
        ...
        if self.state_dict["is_goal_reached"][-1]:  # Goal region is reached
            is_goal_reached = 1
            if self.terminate_on_goal_reached:
                done = True
        elif self.state_dict["is_off_road"][-1]:  # Ego vehicle is off-road
            is_off_road = 1
            if self.terminate_on_off_road:
                done = True
        elif self.state_dict["is_collision"][-1]:  # Collision with others
            is_collision = 1
            if self.terminate_on_collision:
                done = True
        elif self.state_dict["is_time_out"][-1]:  # Max simulation time step is reached
            is_time_out = 1
            if self.terminate_on_time_out:
                done = True
        elif self.state_dict["is_friction_violation"][
            -1
    ...
    def render(self, mode: str = "human") -> None:
        ...
        draw_object(
            ...
            draw_params={
                ...
                "scenario": {
                    "lanelet_network": {
                        "lanelet": {"show_label": False, "fill_lanelet": True}
                    }
                },
            },
        )
        ...
        # Draw ego vehicle
        crdc_draw_dispatch.draw_object(
            self.ego_vehicle.collision_object,
        ...
        if self.render_ego_lanelet_center_vertices:
            ego_lanelet_center_vertices = (
                self.scenario.lanelet_network.find_lanelet_by_id(
                    self.state_dict["ego_vehicle_lanelet_id"][-1]
                ).center_vertices
            )
            plt.plot(
        ...
        if (
            ...
            and self.render_lane_rect_surrounding_area
            and not self.terminated
        ):
            crdc_draw_dispatch.draw_object(
                self.state_dict["lane_rect_surrounding_area"][-1],
                draw_params={"collision": {"facecolor": "None"}},
            )
        if (
            self.observe_lane_rect_surrounding
            and self.render_lane_rect_surrounding_obstacles
            and not self.terminated
        ):
            colors = ["r", "r", "y", "y", "k", "k"]
            for obs, color in zip(
                self.state_dict["lane_rect_obs"][self.current_step], colors
            ):
                if obs is not None:
                    plt.plot(
                        obs.position[0],
        ...
        if (
            ...
            and self.render_lane_circ_surrounding_area
            and not self.terminated
        ):
            crdc_draw_dispatch.draw_object(
                self.state_dict["lane_circ_surrounding_area"][-1],
                draw_params={"collision": {"facecolor": "None"}},
            )
        if (
            self.observe_lane_circ_surrounding
            and self.render_lane_circ_surrounding_obstacles
            and not self.terminated
        ):
            colors = ["r", "r", "y", "y", "k", "k"]
            for obs, color in zip(
                self.state_dict["lane_circ_obs"][self.current_step], colors
            ):
                if obs is not None:
                    plt.plot(
                        obs.position[0],
        ...
        if (
            self.observe_lidar_elli_surrounding
            and self.render_lidar_elli_surrounding_beams
            and not self.terminated
        ):
            for (beam_start, beam_length, beam_angle) in self.state_dict[
                "lidar_elli_surrounding_beams"
            ][-1]:
                center = beam_start + 0.5 * beam_length * approx_orientation_vector(
                    beam_angle
                )
                beam_angle = make_valid_orientation(beam_angle)
                beam_draw_object = Rectangle(
                    length=beam_length, width=0.1, center=center, orientation=beam_angle
                )
                draw_object(beam_draw_object)
        if (
            self.observe_lidar_elli_surrounding
            and self.render_lidar_elli_surrounding_obstacles
            and not self.terminated
        ):
            for idx, detection_point in enumerate(
                self.state_dict["lidar_elli_obs"][-1]
            ):
                plt.plot(
                    detection_point[0],
                    detection_point[1],
        ...
        if (
            self.observe_static_extrapolated_positions
            and self.render_static_extrapolated_positions
        ):
            for future_pos in self.state_dict["extrapolation_static_pos"][-1]:
                plt.plot(future_pos[0], future_pos[1], color="r", marker="x", zorder=21)
        if (
            self.observe_dynamic_extrapolated_positions
            and self.render_dynamic_extrapolated_positions
        ):
            for future_pos in self.state_dict["extrapolation_dynamic_pos"][-1]:
    ...
    def _build_observation_space(self) -> None:
        ...
        if self.observe_v_ego:
            observation_space_dict["v_ego"] = gym.spaces.Box(
                -np.inf, np.inf, (1,), dtype=np.float32
            )
        if self.observe_a_ego:
            observation_space_dict["a_ego"] = gym.spaces.Box(
                -np.inf, np.inf, (1,), dtype=np.float32
            )
        if self.observe_steering_angle:
            observation_space_dict["steering_angle"] = gym.spaces.Box(
                -np.pi, np.pi, (1,), dtype=np.float32
            )
        if self.observe_heading:
            observation_space_dict["heading"] = gym.spaces.Box(
                -np.pi, np.pi, (1,), dtype=np.float32
            )
        if self.observe_global_turn_rate:
            observation_space_dict["global_turn_rate"] = gym.spaces.Box(
                -np.inf, np.inf, (1,), dtype=np.float32
            )
        if self.observe_left_marker_distance:
            observation_space_dict["left_marker_distance"] = gym.spaces.Box(
                -np.inf, np.inf, (1,), dtype=np.float32
            )
        if self.observe_right_marker_distance:
            observation_space_dict["right_marker_distance"] = gym.spaces.Box(
                -np.inf, np.inf, (1,), dtype=np.float32
            )
        if self.observe_left_road_edge_distance:
            observation_space_dict["left_road_edge_distance"] = gym.spaces.Box(
                -np.inf, np.inf, (1,), dtype=np.float32
            )
        if self.observe_right_road_edge_distance:
            observation_space_dict["right_road_edge_distance"] = gym.spaces.Box(
                -np.inf, np.inf, (1,), dtype=np.float32
            )
        if self.observe_lat_offset:
            observation_space_dict["lat_offset"] = gym.spaces.Box(
                -np.inf, np.inf, (1,), dtype=np.float32
            )

        # Goal-related
        if self.observe_distance_goal_long:
            observation_space_dict["distance_goal_long"] = gym.spaces.Box(
                -np.inf, np.inf, (1,), dtype=np.float32
            )
        ...
        if self.observe_distance_goal_lat:
            observation_space_dict["distance_goal_lat"] = gym.spaces.Box(
                -np.inf, np.inf, (1,), dtype=np.float32
            )
        ...
        if self.observe_distance_goal_long_lane:
            observation_space_dict["distance_goal_long_lane"] = gym.spaces.Box(
                -np.inf, np.inf, (1,), dtype=np.float32
            )
        ...
        if self.observe_lane_rect_surrounding:
            observation_space_dict["lane_rect_v_rel"] = gym.spaces.Box(
                -np.inf, np.inf, (6,), dtype=np.float32
            )
            observation_space_dict["lane_rect_p_rel"] = gym.spaces.Box(
                -self.dummy_dist, self.dummy_dist, (6,), dtype=np.float32
            )
        # # Lane-based circular surrounding observation
        if self.observe_lane_circ_surrounding:
            observation_space_dict["lane_circ_v_rel"] = gym.spaces.Box(
                -np.inf, np.inf, (6,), dtype=np.float32
            )
            observation_space_dict["lane_circ_p_rel"] = gym.spaces.Box(
                -self.dummy_dist, self.dummy_dist, (6,), dtype=np.float32
            )
        # # Lidar-based elliptical surrounding observation
        if self.observe_lidar_elli_surrounding:
            num_beams = self.lidar_elli_num_beams
            observation_space_dict["lidar_elli_dist_rate"] = gym.spaces.Box(
                -np.inf, np.inf, (num_beams,), dtype=np.float32
            )
            observation_space_dict["lidar_elli_dist"] = gym.spaces.Box(
                -self.dummy_dist, self.dummy_dist, (num_beams,), dtype=np.float32
            )

        # Termination-related
        if self.observe_remaining_steps:
            observation_space_dict["remaining_steps"] = gym.spaces.Box(
                -np.inf, np.inf, (1,), dtype=np.float32
            )
        if self.observe_is_time_out:
            observation_space_dict["is_time_out"] = gym.spaces.Box(
                0, 1, (1,), dtype=np.int8
            )
        if self.observe_is_collision:
            observation_space_dict["is_collision"] = gym.spaces.Box(
                0, 1, (1,), dtype=np.int8
            )
        if self.observe_is_off_road:
            observation_space_dict["is_off_road"] = gym.spaces.Box(
                0, 1, (1,), dtype=np.int8
            )
        if self.observe_is_friction_violation:
            observation_space_dict["is_friction_violation"] = gym.spaces.Box(
                0, 1, (1,), dtype=np.int8
            )
        if self.observe_is_goal_reached:
            observation_space_dict["is_goal_reached"] = gym.spaces.Box(
                0, 1, (1,), dtype=np.int8
            )
        ...
        if self.flatten_observation:
            self.observation_space_size = sum(
                [np.prod(i.shape) for i in self.observation_space_dict.values()]
            )
            self.observation_space = gym.spaces.Box(
                -np.inf, np.inf, (self.observation_space_size,), dtype=np.float32
            )
            LOGGER.debug(
                f"Size of flattened observation space: {self.observation_space_size}"
            )
        else:
            self.observation_space = gym.spaces.Dict(self.observation_space_dict)
            LOGGER.debug(
                f"Length of dictionary observation space: {len(self.observation_space_dict)}"
            )
    ...
    def _set_scenario_problem(self) -> None:
        ...
        # Set episode length
        self.ep_length = max(
            s.time_step.end for s in self.planning_problem.goal.state_list
        )
        ...
        full_reset = False
        cc_template = self.cache_collision_checker_templates.get(
    ...
    def _set_ego_vehicle(self) -> None:
        ...
        if not self.ego_vehicle:
            self.ego_vehicle = Vehicle.create_vehicle(VEHICLE_PARAMS)
    ...
    def _get_observation(self) -> Union[np.ndarray, Dict]:
        ...
        state_dict contains hidden states over all time steps, which help monitor scenarios or optimize configurations
        ...
        lanelet_polygons = self.cache_lanelet_polygons_accel_struct.get(
            self.meta_scenario_id, None
        )
        lanelet_polygons_sg = self.cache_lanelet_polygons_sg_accel_struct.get(
            self.meta_scenario_id, None
        )
        if lanelet_polygons is None:
            ...
            self.cache_lanelet_polygons_sg_accel_struct[
                self.meta_scenario_id
            ] = lanelet_polygons_sg
            self.cache_lanelet_polygons_accel_struct[
                self.meta_scenario_id
            ] = lanelet_polygons
        ...
        if len(ego_vehicle_lanelet_ids) == 0:
            ego_vehicle_lanelet_id = self.state_dict["ego_vehicle_lanelet_id"][-1]
        else:
            ego_vehicle_lanelet_id = ego_vehicle_lanelet_ids[0]
        self.state_dict["ego_vehicle_lanelet_id"].append(ego_vehicle_lanelet_id)
        ego_vehicle_lanelet = self.scenario.lanelet_network.find_lanelet_by_id(
        ...
        if self.current_step == 0:
            ...
            is_off_road = self._check_off_road(self.ego_vehicle)
            is_collision = (
                self.state_dict["is_collision"][self.current_step - 1]
                or self._check_collision()
            )
            is_time_out = self.current_step >= self.ep_length and not is_goal_reached
            is_friction_violation = self.ego_vehicle._check_friction_violation(
                self.ego_vehicle.params, self.ego_vehicle.previous_state
            )

        self.state_dict["is_goal_reached"].append(is_goal_reached)
        self.state_dict["is_off_road"].append(is_off_road)
        self.state_dict["is_collision"].append(is_collision)
        self.state_dict["is_time_out"].append(is_time_out)
        self.state_dict["is_friction_violation"].append(is_friction_violation)
        ...
        if (
            (is_goal_reached and self.terminate_on_goal_reached)
            ...
            is_ego_lanelet_changed = len(
                self.state_dict["ego_vehicle_lanelet_id"]
            ) == 1 or (
                self.state_dict["ego_vehicle_lanelet_id"][-1]
                != self.state_dict["ego_vehicle_lanelet_id"][-2]
                and self.state_dict["ego_vehicle_lanelet_id"][-1] != -1
            )
            if is_ego_lanelet_changed:
                ...
                left_marker_line, right_marker_line = get_lane_marker(
                    ego_vehicle_lanelet
                )
                ...
                lanelet_dict, all_lanelets_set = get_nearby_lanelet_id(
                    self.connected_lanelet_dict, ego_vehicle_lanelet
                )
            ...
            if self.observe_steering_angle:
                observation_dict["steering_angle"] = np.array(
                    [ego_vehicle_state.steering_angle]
                )
            if self.observe_heading:
                heading = get_lane_relative_heading(
                    ego_vehicle_state, ego_vehicle_lanelet
                )
                self.state_dict["heading"].append(heading)
                observation_dict["heading"] = np.array([heading])

            if self.observe_global_turn_rate:
                observation_dict["global_turn_rate"] = np.array(
                    [ego_vehicle_state.yaw_rate]
                )
            if self.observe_lat_offset:
                lat_offset = get_relative_offset(
                    self.local_curvi_cosy, ego_vehicle_state.position
                )
                if np.isnan(lat_offset):
                    # note that we (reasonably) assume that the ego vehicle starts inside the ccosy
                    assert (
                        len(self.state_dict["lat_offset"]) > 0
                    ), "Ego vehicle started outside the local coordinate system"
                    lat_offset = self.state_dict["lat_offset"][-1]
                self.state_dict["lat_offset"].append(lat_offset)
            ...
            if (
                ...
                (
                    ...
                    ego_vehicle_state,
                    self.surrounding_dict["left_marker_line"],
                ...
                if self.observe_left_marker_distance:
                    observation_dict["left_marker_distance"] = np.array(
                        [left_marker_distance]
                    )
                if self.observe_right_marker_distance:
                    observation_dict["right_marker_distance"] = np.array(
                        [right_marker_distance]
                    )
                if self.observe_left_road_edge_distance:
                    observation_dict["left_road_edge_distance"] = np.array(
                        [left_road_edge_distance]
                    )
                if self.observe_right_road_edge_distance:
                    observation_dict["right_road_edge_distance"] = np.array(
                        [right_road_edge_distance]
                    )
            ...
            if self.observe_distance_goal_long or self.observe_distance_goal_lat:
                ...
                if np.isnan(distance_goal_long):
                    assert (
                        len(self.state_dict["distance_goal_long"]) > 0
                    ), "Ego vehicle started outside the global coordinate system"
                    distance_goal_long = self.state_dict["distance_goal_long"][-1]
                    distance_goal_lat = self.state_dict["distance_goal_lat"][-1]
                if self.observe_distance_goal_long:
                    self.state_dict["distance_goal_long"].append(distance_goal_long)
                    observation_dict["distance_goal_long"] = np.array(
                        [distance_goal_long]
                    )
                    if self.current_step == 0:
                        ...
                        distance_goal_long_advance = (
                            self.state_dict["distance_goal_long"][-2]
                            - self.state_dict["distance_goal_long"][-1]
                        )
                    self.state_dict["distance_goal_long_advance"].append(
                        distance_goal_long_advance
                    )
                    observation_dict["distance_goal_long_advance"] = np.array(
                        [distance_goal_long_advance]
                    )
                if self.observe_distance_goal_lat:
                    self.state_dict["distance_goal_lat"].append(distance_goal_lat)
                    observation_dict["distance_goal_lat"] = np.array(
                        [distance_goal_lat]
                    )
                    if self.current_step == 0:
                        ...
                        distance_goal_lat_advance = (
                            self.state_dict["distance_goal_lat"][-2]
                            - self.state_dict["distance_goal_lat"][-1]
                        )
                    self.state_dict["distance_goal_lat_advance"].append(
                        distance_goal_lat_advance
                    )
                    observation_dict["distance_goal_lat_advance"] = np.array(
                        [distance_goal_lat_advance]
                    )

            if self.observe_distance_goal_long_lane:
                distance_goal_long_lane = self.goal.get_long_distance_until_lane_change(
                    ego_vehicle_state, ego_vehicle_lanelet_ids
                )
                if np.isnan(distance_goal_long_lane):
                    assert (
                        len(self.state_dict["distance_goal_long_lane"]) > 0
                    ), "Ego vehicle started outside the global coordinate system"
                    distance_goal_long_lane = self.state_dict[
                        "distance_goal_long_lane"
                    ][-1]
                self.state_dict["distance_goal_long_lane"].append(
                    distance_goal_long_lane
                )
                observation_dict["distance_goal_long_lane"] = np.array(
                    [distance_goal_long_lane]
                )
            ...
            if self.observe_static_extrapolated_positions:
                ...
                for i in range(len(static_lat_offset)):
                    if np.isnan(static_lat_offset[i]):
                        if len(self.state_dict["extrapolation_static_off"]) > 0:
                            # We can assume that the ego vehicle starts inside the ccosy,
                            # however not that any extrapolated position is inside the ccosy
                            static_lat_offset[i] = self.state_dict[
                                "extrapolation_static_off"
                            ][-1][i]
                        else:
                            static_lat_offset[i] = 0
                self.state_dict["extrapolation_static_off"].append(
                    np.array(static_lat_offset)
                )
                self.state_dict["extrapolation_static_pos"].append(np.array(static_pos))
                observation_dict["extrapolation_static_off"] = np.array(
                    static_lat_offset
                )

            if self.observe_dynamic_extrapolated_positions:
                sampling_points = self.dynamic_extrapolation_samples
                dynamic_lat_offset, dynamic_pos = get_relative_future_goal_offsets(
                    self.goal, ego_vehicle_state, sampling_points, static=False
                )
                # Fix nans for positions outside the ccosy
                for i in range(len(dynamic_lat_offset)):
                    if np.isnan(dynamic_lat_offset[i]):
                        if len(self.state_dict["extrapolation_dynamic_off"]) > 0:
                            # We can assume that the ego vehicle starts inside the ccosy,
                            # however not that any extrapolated position is inside the ccosy
                            dynamic_lat_offset[i] = self.state_dict[
                                "extrapolation_dynamic_off"
                            ][-1][i]
                        else:
                            dynamic_lat_offset[i] = 0
                self.state_dict["extrapolation_dynamic_off"].append(
                    np.array(dynamic_lat_offset)
                )
                self.state_dict["extrapolation_dynamic_pos"].append(
                    np.array(dynamic_pos)
                )
                observation_dict["extrapolation_dynamic_off"] = np.array(
                    dynamic_lat_offset
                )
            ...
            if self.observe_lane_rect_surrounding:
                ...
                self.state_dict["lane_rect_obs"].append(detected_obstacle)
                self.state_dict["lane_rect_surrounding_area"].append(surrounding_area)
            ...
            if self.observe_lane_circ_surrounding:
                ...
                self.state_dict["lane_circ_obs"].append(detected_obstacle)
                self.state_dict["lane_circ_surrounding_area"].append(surrounding_area)
            ...
            if self.observe_lidar_elli_surrounding:
                ...
                self.state_dict["lidar_elli_obs"].append(detection_points)
                self.state_dict["lidar_elli_obs_detection"].append(
                    [1 if x != self.dummy_dist else 0 for x in dist]
                )
                self.state_dict["lidar_elli_surrounding_beams"].append(
                    surrounding_beams
                )
            ...
            # Termination-related
            if self.observe_remaining_steps:
                observation_dict["remaining_steps"] = np.array(
                    [self.ep_length - self.current_step]
                )
            if self.observe_is_goal_reached:
            ...
            if self.observe_is_friction_violation:
                observation_dict["is_friction_violation"] = np.array(
        ...
        if self.flatten_observation:
            ...
            index = 0
            for k in sorted(self.observation_space_dict.keys()):
    ...
    def _check_off_road(self, ego_vehicle: Vehicle) -> bool:
        """
        ...
        if self.strict_off_road_check == True:
            ...
            collision_ego_vehicle_center = pycrcc.Circle(
                0.5, ego_vehicle.state.position[0], ego_vehicle.state.position[1]
            )
    ...
    def _get_road_edge(
        self, ego_vehicle_lanelet_id: int
        ...
        """
        left_most_lanelet_id = self.road_edge["left_road_edge_lanelet_id_dict"][
            ego_vehicle_lanelet_id
        ]
        right_most_lanelet_id = self.road_edge["right_road_edge_lanelet_id_dict"][
            ego_vehicle_lanelet_id
        ]
    ...
    def _get_reward(self) -> float:
        ...
        reward += self.reward_reverse_driving * (
            self.ego_vehicle.state_list[-1].velocity < 0.0
        )
        ...
        if self.reward_type == "dense_reward":
            ...
            if len(rel_pos) == 0:
                ...
                r_obs_lane = (
                    self.reward_obs_distance_coefficient
            ...
            if len(dist) == 0:
                ...
                r_obs_lidar = (
                    self.reward_obs_distance_coefficient
                    * np.sum(dist)
                    / (self.dummy_dist * len(dist))
                )

            # Calculate normalized distance to goal as a negative reward
            dist_goal = np.sqrt(
                self.observation_dict["distance_goal_long"][0] ** 2
