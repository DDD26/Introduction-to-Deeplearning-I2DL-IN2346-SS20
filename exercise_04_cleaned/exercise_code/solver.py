...
class Solver(object):
    ...
    def _step(self):
        ...
        hat_y, dhat_y_dW = model(X_train)
        Loss, dLoss_dhat_y = loss_func(hat_y, y_train)
        dLoss_dW = np.zeros(dhat_y_dW.shape)
        dLoss_dW = np.dot(dLoss_dhat_y.T, dhat_y_dW) / y_train.shape[0]
        self.model.W = opt.step(dLoss_dW.T)
