...
class Vehicle(ABC):
    ...
    def create_vehicle(params_dict: dict) -> "Vehicle":
        ...
        """
        vehicle_type = params_dict["vehicle_type"]
        vehicle_model = params_dict["vehicle_model"]
        if vehicle_model == VehicleModel.PM:
            raise NotImplementedError(
                f"This vehicle model is not supported yet: {vehicle_model}"
            )
        ...
        elif vehicle_model is None:
            vehicle = _VehicleOld(params_dict)
    ...
    def _update_state(self, new_state: State) -> None:
        ...
        current_state.yaw_rate = new_state.yaw_rate
...
class _VehicleOld(Vehicle):
    ...
    def __init__(self, vehicle_params: dict) -> None:
        super().__init__(None)
        self.length = vehicle_params["length"]
        self.width = vehicle_params["width"]
        self.a_max = vehicle_params["acc_max"]
        self.turn_rate_max = vehicle_params["turn_rate_max"]
        self.current_time_step = 0
        self.turn_rate = 0.0
        self.l_wb = vehicle_params["l_wb"]
    ...
    def rescale_action(self, normalized_action: np.ndarray) -> np.ndarray:
        ...
        :return: rescaled action
        """
        factor = np.array([self.a_max, self.turn_rate_max])
